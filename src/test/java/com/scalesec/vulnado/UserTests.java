...
class UserTest {
    ...
    void fetch_ShouldPrintQueryToConsole() throws Exception {
        ...
        assertTrue(outContent.toString().contains("select * from users where username = ? limit 1"), "Fetch should print the prepared statement to console");
    ...
    @Test
    void fetch_ShouldUsePreparedStatementToPreventSQLInjection() throws Exception {
        ...
        verify(mockPreparedStatement).setString(1, maliciousUsername);
        verify(mockPreparedStatement, never()).executeQuery(contains(maliciousUsername));
    }
    ...
    void token_WithEmptySecret_ShouldThrowException() {
        assertThrows(IllegalArgumentException.class, () -> testUser.token(""), "token method should throw IllegalArgumentException for empty secret");
    }

    @Test
    void assertAuth_WithEmptySecret_ShouldThrowException() {
        String token = testUser.token(TEST_SECRET);
        assertThrows(IllegalArgumentException.class, () -> User.assertAuth("", token), "assertAuth should throw IllegalArgumentException for empty secret");
    }
}
